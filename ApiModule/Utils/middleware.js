...
const logging = (req, res, next) => {
  let ip = req.ip;
  if (ip === "::1") {
    ip = process.env.OFFICE_IP;
  }
  console.log(
    color(
      `Requisição: ${req.method} ${req.url} [ ${ip} ] - ${new Date()}`,
      "blue"
    )
  );
  next();
...
const isAPIAlreadyUsed = (key) => {
  // Não é middleware, é uma função para verificar se a API já existe
...
const isAdmin = (userId) => {
  // Não é middleware, é uma função para verificar se o utilizador é admin
  return new Promise((resolve, reject) => {
    appAdminModel
      .findOne({ userId })
      .then((result) => {
